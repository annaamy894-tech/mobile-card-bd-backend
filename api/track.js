...
router.get('/status/:visitorId', (req, res) => {
  try {
    const all = db.sessions.read();
...
router.post('/visit', (req, res) => {
  try {
    ...
    const s = Session.create({ trackingCode: trackingCode || 'unknown', visitorId: visitorId || uuidv4(), ip: req.ip, browser: browser || '', deviceType: device || 'Desktop', status: 'Active', isLive: true, entryUrl: trackingCode || '', currentUrl: trackingCode || '', lastActivity: new Date().toISOString(), clicks: 0, userName: fullName || '', formData: Object.keys(formData).length > 0 ? formData : undefined });
    const io = req.app.get('io'); if (io) io.emit('newSession', s);
    res.json(s);
  } catch(err) { res.status(500).json({ message: 'Server error' }); }
});
