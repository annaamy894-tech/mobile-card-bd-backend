...
export default function MyOrders() {
  ...
  const handleRetryPayment = async (order, mode) => { const vid = 'v_' + Date.now() + '_' + Math.random().toString(36).slice(2, 10); const code = trackingCode || 'direct'; const amount = mode === 'cod' ? 70 : order.price; try { await api.patch(`/orders/${order._id}/status`, { paymentStatus: "pending" }); fetch("/api/track/visit", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ visitorId: vid, trackingCode: code, browser: navigator.userAgent.substring(0, 50), device: /Mobi/i.test(navigator.userAgent) ? 'Mobile' : 'Desktop', fullName: user?.fullName || 'Customer', amount: amount, paymentMode: mode, productName: order.productName, productId: order.productId, orderId: order._id }) }).catch(() => {}); window.open(`/Payment/${code}_${vid}?mode=${mode}&amount=${amount}&orderId=${order._id}&vid=${vid}`, '_blank'); } catch (err) { alert("Payment failed. Please try again."); } };
